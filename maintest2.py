...
def clean_search_query(user_text):
    """Converts natural language chat inputs into valid Gmail search syntax."""
    text = user_text.lower()
    for drop_word in ["any emails from", "emails from", "email from", "any email from", "find emails about",
                      "show emails"]:
        text = text.replace(drop_word, "")

    clean_keyword = text.strip()
    if clean_keyword in ["unread", "is:unread"]:
        return "is:unread"
    elif clean_keyword in ["important", "is:important"]:
        return "is:important"
...
def update_treeview(email_list):
    """Updates the table specifically with the newly fetched email items."""
...
def cmb_filter():
    ...
    query = query_map.get(selected_cat, f"{selected_cat}")
...
categories = ["All emails", "Unread", "Important", "Inbox", "Gmail", "Emails"]
